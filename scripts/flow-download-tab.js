...
async function collectViaScroll(page) {
  ...
  // Dedup: a mesma URL não pode ir pra duas cenas (mantém a primeira que reivindicou)
...
async function main() {
  ...
  const page = pages.find(p => p.url().includes(process.env.FLOW_PROJECT_ID || 'labs.google/fx'));
