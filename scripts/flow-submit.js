...
async function submitPrompt(page, prompt) {
  try {
    ...
    // Submeter com Enter (Glitch pediu Enter explicitamente).
    await page.keyboard.press('Enter');
    await sleep(400);
    // Verifica se editor esvaziou (sinal de que submeteu)
    const submitted = await page.evaluate(() => {
      const ed = document.querySelector('div[data-slate-editor="true"]');
      return ed ? ed.textContent.length < 10 : false;
    });
    if (!submitted) {
      // Fallback: clicar botão Criar/Create (UI suporta PT e EN)
      const clicked = await page.evaluate(() => {
        const buttons = document.querySelectorAll('button');
        // Prioridade 1: arrow_forward + Criar/Create (botão de submit principal)
        for (const btn of buttons) {
          if (btn.textContent.includes('arrow_forward') && (btn.textContent.includes('Criar') || btn.textContent.includes('Create')) && !btn.disabled && btn.offsetParent) { btn.click(); return true; }
        }
        // Prioridade 2: Criar/Create sem ser add_2 (que é "Add prompt" não "Submit")
        for (const btn of buttons) {
          if ((btn.textContent.includes('Criar') || btn.textContent.includes('Create')) && !btn.disabled && btn.offsetParent && !btn.textContent.includes('add_2')) { btn.click(); return true; }
        }
        return false;
      });
      if (clicked) process.stdout.write(`  ✅ Cena ${String(prompt.num).padStart(3,'0')} (${textLen} chars · clicked)\n`);
      return clicked;
    }
    process.stdout.write(`  ✅ Cena ${String(prompt.num).padStart(3,'0')} (${textLen} chars · enter)\n`);
    return true;
  } catch (err) {
...
async function main() {
  ...
  let page = pages.find(p => /labs\.google.*\/flow\/project\//.test(p.url()))
